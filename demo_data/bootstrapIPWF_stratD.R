...
set.seed(seeds[m])
  ...
  #ipwdat2 = ipwdat22; ipwdat2$pi = ipwdat2$pred_obs; ipwdat2$pi = ifelse(is.na(ipwdat2$pek), NA, ipwdat2$pi); ipwdat2$pred_obs=NULL;
  ...
  return(c(summary(ipw.fitd)$coef[,1]))
}
test = foreach(m=1:1000) %dopar% myfunc(m)
test2 = do.call("rbind", test)

write.csv(test2,"listipwd.csv")
