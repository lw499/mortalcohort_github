...
myfunc = function(m)
{
  ...
  fit = geeglm(pek ~  factor(time),id=Case,data=tmpdata,corstr="ind")
...
ipw.fit = geeglm(pek ~  factor(time), data = ripwdat, weights = pi, id=Case)
...
ipw.fitd = geeglm(pek ~ factor(time), data = ripwdatd, weights = pi, id=Case)
...
for(i in 1:nsim)
{
  ...
  fitmi = geeglm(pek ~  factor(time), data = midatd, id=id,na.action=na.omit)
...
for(i in 1:nsim)
{
  ...
  fitmi = geeglm(pek ~  factor(time), data = midat, id=id,na.action=na.omit)
  PE[i,]=summary(fitmi)$coef[,1]
}
...
fitli = geeglm(pek ~ factor(time), id=Case, data = lidat)
